...
import { getPosts, countCategories, categories } from "@/lib/posts";

export const POSTS_PER_PAGE = 9;

// Page 1 lives at /blog; pages 2+ live at /blog/page/[page] so both can be
// statically generated (a route can't self-reference with a query string).
  ...
  return (
    <main className="min-h-screen">
      <div className="max-w-[750px] mx-auto px-7 pt-9 pb-16">
        ...
        <h1 className="font-heading text-[34px] font-normal leading-[1.4] tracking-[-0.005em] mb-8">The journal.</h1>
        ...
        {totalPages > 1 && (
          ...
          >
            {currentPage > 1 && (
              <Link href={pageHref(currentPage - 1)} className="text-ink/50 hover:text-ink/80 px-2 transition-colors" aria-label="Previous page">
                &larr;
              </Link>
            )}
            ...
            {currentPage < totalPages && (
              <Link href={pageHref(currentPage + 1)} className="text-ink/50 hover:text-ink/80 px-2 transition-colors" aria-label="Next page">
                &rarr;
              </Link>
            )}
          </nav>
